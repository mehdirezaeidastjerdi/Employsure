...
# Purpose: Find inactive Entra ID users using Microsoft Graph API and generate a report
# Author: Mehdi Rezaei with help of AI
# Last Modified: 18/09/2025
# **********************************************************************************************************************
# This PowerShell script utilizes the Microsoft Graph API to identify inactive Entra ID users within a specified time frame. 
# The script prompts for the Entra ID tenant name and the desired duration of user inactivity. It then retrieves user data, 
...
$emailParams = @{
    From        = $senderEmail
    To          = $receiverEmail
    Subject     = "Inactive Entra ID Users Report"
    Body        = @"
Hello Team,

Please find attached the latest Inactive Entra ID Users Report generated by the automated script.

This report lists all Entra ID user accounts that have not signed in for the past 30 days or have never signed in. 
It includes information such as Display Name, User Principal Name, Account Creation Date, and Last Sign-In Activity.

This information can help us identify accounts that may need to be reviewed, disabled, or removed to maintain a secure and up-to-date directory.
